...
start_text = """👋Всем привет, меня зовут Виктор и в этом боте я буду с вами делиться информацией, как можно заработать на IEO на самой популярной бирже в СНГ. Я расскажу вам тактику, на которой я лично зарабатываю уже несколько лет. Благодаря этому вы сможете увеличивать свой капитал в разы.
...
second_message = """💰Как будем зарабатывать на IEO? Как учавствовать в IEO?
...
foto2 = open('./message2.jpg', 'rb')

bot = telebot.TeleBot('*******************')
admins=[359297087, 60201964]
...
def send_message_admins( message):
    for id in admins:
        bot.send_message(id , message)
...
def mailing(message):
    if (message.chat.id in admins):
        ...
        bot.register_next_step_handler(message, send_content)
...
def send_text(message):
     f = open('all_id', 'r')
     for id in f:
        send_message(int(id), str(message.text))
...
def check_photo(message):
    if message.content_type == 'photo':
        ...
        bot.send_photo(message.chat.id, foto2, )
    else:
